...
<?php 
	...
	if(isset($_POST['ajax'])) {		
       ...
       <div class="container">
         <address>
			<div class="row <?php if( !$smof_data['rnr_enable_googlemap']) { echo 'centered-map';} ?>">
				<div class="span12">
					<?php if( $smof_data['rnr_contact_email']) { ?><p class="e-mail"><?php echo $smof_data['rnr_contact_email']; ?></p> <?php } ?>
					<?php if( $smof_data['rnr_contact_phone']) { ?><p class="telephone"><?php echo $smof_data['rnr_contact_phone']; ?></p> <?php } ?>
					<?php if( $smof_data['rnr_contact_address']) { ?><p class="address"><?php echo $smof_data['rnr_contact_address']; ?></p> <?php } ?>
				</div>
			</div>	
		</address>
            <div class="row <?php if( !$smof_data['rnr_enable_googlemap']) { echo 'centered-map';} ?>">
             <?php if( $smof_data['rnr_enable_googlemap']) { ?>          
              <div  class="span7 contact-map">
                      ...
                      function initialize() {
                              ...
                              var myMapOptions = {
                                  ...
                                  ,zoom: <?php echo $smof_data['rnr_map_zoom']; ?> , scrollwheel: false,mapTypeControl: false, zoomControl: true
                              };
             ...
             <?php } ?> 
              ...
              <div class="span5">                
				...
				<form action="<?php the_permalink(); ?>" method="post" class="contactForm form" id="contact-form">		
                              ...
                              <input type="text" name="contactSubject" id="contactSubject" value="<?php _e('Subject', 'rocknrolla'); ?>" />
                              </div>
